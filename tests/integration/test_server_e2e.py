...
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    When run locally without GCP credentials, the adapter returns 503 (expected).
    """
    response = requests.post(
    ...
    # 503 is acceptable locally (no GCP credentials); 200 expected in production.
    assert response.status_code in (200, 503), (
        f"Unexpected status {response.status_code}: {response.text[:200]}"
    )

    if response.status_code == 503:
        logger.info("Skipping streaming assertions: reasoning_engine adapter unavailable (no GCP credentials)")
        return
