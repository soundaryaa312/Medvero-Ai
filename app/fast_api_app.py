...
try:
    ...
    import logging
    logging.getLogger(__name__).warning(f"GCP credentials not available, falling back to standard logging. Error: {e}")
    gcp_logger = None
...
class SafeLogger:
    ...
    def log_struct(self, info: dict, severity: str = "INFO"):
        if self.original_logger:
            try:
                self.original_logger.log_struct(info, severity=severity)
                return
            except Exception:
                pass
        import logging
        level = getattr(logging, severity.upper(), logging.INFO)
        logging.getLogger(__name__).log(level, f"Structured log: {info}")

logger = SafeLogger(gcp_logger)
...
app.title = "medvero-ai"
app.description = "API for interacting with the Agent medvero-ai"
